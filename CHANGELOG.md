...
- The comment that offered a cut proposal for approval is rewritten to say the change was withdrawn, rather than claiming a decision is pending forever.
- Surviving messages keep their mirrored comment ids, so a cut no longer re-posts the whole thread to a fresh issue.
- A rewind or retry refuses while a GitHub sync is mid-pass, which could otherwise delete a comment the sync had just ingested from a phone before anyone saw it arrive.
